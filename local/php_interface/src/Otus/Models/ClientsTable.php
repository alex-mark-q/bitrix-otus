...
class ClientsTable extends DataManager
{
	...
	public static function getMap()
	{
		return [
			...
			(new Reference(
				'otus_doctors_hospital',
				Hospital::class,
				Join::on('this.doctor_id', 'ref.doctor_id')
			))->configureJoinType('inner'),

			// Связь ManyToMany с больницами через промежуточную таблицу otus_about_doctors
            'hospitals' => (new ManyToMany('hospitals', Hospital::class))
                ->configureTableName('otus_about_doctors') // промежуточная таблица
                ->configureLocalPrimary('id', 'doctor_id') // из основной таблицы докторов
                ->configureRemotePrimary('id', 'doctor_id') // из таблицы больниц
		];
	}
}